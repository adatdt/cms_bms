...
class M_tapout extends CI_Model {
	...
	public function  download()
	{
		...
		$type= decode($this->input->get('type'));
		...
		if (!empty($search))
		{
			$where .="and(  a.plate_number ilike '%".$search."%' or a.route_info  ilike '%".$search."%' or a.uid ilike  '%".$search."%'
						 or a.driver_name ilike  '%".$search."%' or a.driver_phone ilike  '%".$search."%' 
						 or e.bus_name ilike  '%".$search."%' )";
		}
	...
	public function getData()
	{
	    ...
	    foreach ($query->result_array() as $r) {
	    	...
	    	$detail=$this->m_global->menuAccess($this->session->userdata('user_group_id'),'transaction/tap_out','detail');

	    	$edit=$this->m_global->menuAccess($this->session->userdata('user_group_id'),'transaction/tap_out','edit');

	    	$clear=$this->m_global->menuAccess($this->session->userdata('user_group_id'),'transaction/tap_out','force_exit');
	    	...
	    	if ($edit)
	    	{
	    		if($r['status']=='1')
	    		{
				$action .= "<button type='button' class='btn bg-angkasa2 btn-icon btn-xs btn-dtgrid' title='Detail' onclick='edit(".'"'.encode($r['id_seq']).'"'.")' >Edit</button> ";
				}
			}
			...
			if ($clear)
			{
				if($r['status']=='1')
				{
					$action .= "<button type='button' class='btn bg-angkasa2 btn-icon btn-xs btn-dtgrid' title='Detail' onclick='clearData(".'"'.encode($r['id_seq']).'"'.")' >Force Exit</button> ";
				}	
			}

	    	$r['created_on']=format_dateTime($r['created_on']);
	    	$r['action']=$action;
	    	$r['status']=$r['status']=='1'?"<span class='label label-flat border-danger text-danger-600'>Unclear<span>":"<span class='label label-flat border-success text-success-600'>Clear</span>";

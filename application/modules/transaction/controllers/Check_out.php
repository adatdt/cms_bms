...
class Check_out extends MX_Controller {
	...
	public function index()
	{
		...
		$data['content'] = "check_out/index";
		$data['shelter'] = $this->m_global->getData("master.t_mtr_shelter","where status=1 order by shelter_name asc ");

		if($check->num_rows()>0)
		{
			$data['po'] = $this->m_global->getDataById("master.t_mtr_po","id_seq=".$check->row()->po_id." and status=1 ")->row();
			$data['user_po']='1';			
		}
		else
		{
	...
	public function download()
	{
		...
		$data = $this->m_checkout->download()->result();
		$excel = new Exceldownload();
		// Send Header
		$excel->setHeader('Transaction_Check_out.xls');
		$excel->BOF();

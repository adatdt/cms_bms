		...
		<div id="toolbar">
			<div class="row">
				<div class="col col-md-8" style="padding: 5px 15px">
					<?php if ($add) {
						create_btn(site_url('device/auto_gate/add'),'Add','plus-circle2');
					} ?>
				</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('#grid').datagrid({
			url         : '<?php echo site_url('device/auto_gate/getList'); ?>',
			...
			columns:[[
				...
				{ field: 'shelter_name', title: 'Shelter', sortable: true,width:100, halign:"center" },
				// { field: 'terminal_name', title: 'Terminal Device', sortable: true,width:100, halign:"center" },
	...
	function edit(id){
		$.magnificPopup.open({
			items: {
				src: "<?php echo site_url('device/auto_gate/edit/') ?>"+id,
			},
			modal: true,
			type: 'ajax',
	...
	// 	url = "<?php echo site_url('device/auto_gate/delete/') ?>"+id;
	// 	confirm_delete(url);
	// }

	function change_password(id,reload='')
	{
		var url = "<?php echo site_url('device/auto_gate/update_password/') ?>"+id;
		swal({
	...
	{
		var url="<?php echo site_url('device/auto_gate/delete/') ?>"+id+"/"+msg;
		swal({

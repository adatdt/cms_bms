		...
		<div id="toolbar">
			<div class="row">
				<div class="col col-md-8" style="padding: 5px 15px">
					<?php if ($add) {
						create_btn(site_url('device/kiosk/add'),'Add','plus-circle2');
					} ?>
				</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('#grid').datagrid({
			url         : '<?php echo site_url('device/kiosk/getList'); ?>',
	...
	function edit(id){
		$.magnificPopup.open({
			items: {
				src: "<?php echo site_url('device/kiosk/edit/') ?>"+id,
			},
			modal: true,
			type: 'ajax',
	...
	function deleteData(id){
		url = "<?php echo site_url('device/kiosk/delete_data/') ?>"+id;
		confirm_delete(url);
	}

	function change_password(id,reload='')
	{
		var url = "<?php echo site_url('device/kiosk/update_password/') ?>"+id;
		swal({
	...
	{
		var url="<?php echo site_url('device/kiosk/delete/') ?>"+id+"/"+msg;
		swal({

		...
		<div id="toolbar">
			<div class="row">
				<div class="col col-md-8" style="padding: 5px 15px">
					<?php if ($add) {
						create_btn(site_url('device/type/add'),'Add','plus-circle2');
					} ?>
				</div>
...
<script type="text/javascript">
	$("#btndownload").click(function(event){
		...
		window.location.href="<?php echo site_url('transaction/fare/download?') ?>sortFrom="+sortFrom+"&sortTo="+sortTo+"&search="+search;
	});
	...
	$(document).ready(function(){
		$('#grid').datagrid({
			url         : '<?php echo site_url('device/type/getList'); ?>',
			// pagination : true,
			// rownumbers : true,
			// fitColumns : true,
			...
			columns:[[
				...
				{ field: 'terminal_type_name', title: 'Name', sortable: true,width:100 },
	...
	function edit(id){
		$.magnificPopup.open({
			items: {
				src: "<?php echo site_url('device/type/edit/') ?>"+id,
			},
			modal: true,
			type: 'ajax',
	...
	function deleteData(id){
		url = "<?php echo site_url('device/type/delete/') ?>"+id;
		confirm_delete(url);
	}

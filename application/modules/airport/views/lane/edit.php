					...
					<div class="form-group">
						<div class="row" >
							<div class="col-sm-12 form-group">
								...
								<select class="form-control" name="shelter_id" disabled>
									<?php foreach($shelter as $s) {?>
									<option value="<?php echo decode($s->id_seq); ?>" <?php echo $s->id_seq==$lane->shelter_id?"selected":""; ?> ><?php echo $s->shelter_name; ?></option>
									<?php }?>
								</select>
							...
							<div class="col-sm-12 form-group">
								...
								<input type="text" name="lane_name" placeholder="Lane Name" value="<?= $lane->lane_name ?>" autocomplete="off" class="form-control" required>
							</div>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	$("#form_edit").submit(function(event){
		...
		$.ajax({
			url: '<?php echo site_url(); ?>airport/lane/action_edit',
			type: "POST",

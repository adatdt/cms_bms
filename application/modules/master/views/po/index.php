		...
		<div id="toolbar">
			<div class="row">
				<div class="col col-md-8" style="padding: 5px 15px">
					<?php if ($add) {
						create_btn(site_url('master/po/add'),'Add','plus-circle2');
					} ?>
				</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('#grid').datagrid({
			url         : '<?php echo site_url('master/po/getList'); ?>',
			// pagination : true,
			// rownumbers : true,
			// fitColumns : true,
	...
	function edit(id){
		$.magnificPopup.open({
			items: {
				src: "<?php echo site_url('master/po/edit/') ?>"+id,
			},
			modal: true,
			type: 'ajax',
	...
	function deleteData(id){
		url = "<?php echo site_url('master/po/delete/') ?>"+id;
		confirm_delete(url);
	}
